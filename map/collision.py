...
from config import CUSTOMER_SOLID_TILES, SOLID_TILES, TILE_SIZE
...
def get_customer_solid_tiles_around(rect: pygame.Rect, tile_map) -> list[pygame.Rect]:
    """Return solid tile rects for customers (excluding doors so they can pass through)."""
    tiles: list[pygame.Rect] = []
    ...
    for row in range(top, bottom + 1):
        for col in range(left, right + 1):
            if tile_map.tile_at(col, row) in CUSTOMER_SOLID_TILES:
                x = col * TILE_SIZE
                y = row * TILE_SIZE
                tiles.append(pygame.Rect(x, y, TILE_SIZE, TILE_SIZE))
